...
func _init() -> void:
	...
	for world_seed in SEEDS:
		...
		for year in range(YEARS):
			...
			herd_prints[_herd_fingerprint(world)] = true
		...
		if period > 0:
			...
			# Herd-only states reported beside the full count so a pass that rests
			# entirely on vitality digits drifting is visible as one.
			print("ok   seed %d did not settle in %d years (%d distinct year-states, %d from herd positions and sizes alone)"
					% [world_seed, YEARS, distinct.size(), herd_prints.size()])
...
func _fingerprint(world: WorldMap) -> String:
	var parts: Array[String] = [_herd_fingerprint(world)]
...
func _herd_fingerprint(world: WorldMap) -> String:
