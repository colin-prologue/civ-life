...
**Tickets:** to be filed as A (tasks 1–3), B (tasks 4–6), C (tasks 7–8)
...
| `sim/world_map.gd` (modify) | A `households` array beside `nodes`; the reconsideration pass in `advance_turn()`. |
...
|---|---|---|
| `FOLLOW` | toward the highest forage demand within sense range — that is where the animals are, read through the census rather than by asking what anything is | herds are rich and moving |
| `TEND` | stays put unless its tile falls below a stated fraction of its own best remembered value | a place is stable and productive |
...
**Produces:**
- `Household` — `id`, `practice`, `members: Array[Person]`, `last_surplus`, `gathered_this_interval`, `phase_offset()`, `threshold()`, `standing()` (its member count — derived, never stored)
- `Person extends Agent` — `household_id`, `forage_demand()` returning a stated per-person appetite
- `WorldMap.households: Array[Household]`, `add_household()`, and the same frozen-census discipline for anything a person divides
...
- [ ] **Step 1: failing tests.** A Tending person wears `TEND` and leaves `GRAZE` untouched; a person on worn ground takes less than one on fresh ground; two identical co-located households end a turn identical; a tile with more people on it yields each of them less (FR-8c).
- [ ] **Step 2: implement.**
- [ ] **Step 3: verify.**
...
|---|---|---|
| **A** | 1–3 | Pure additions: a hash, a policy table, three unused vitality rows. Nothing in the world behaves differently, and the budget question is answered here rather than inside a larger change. |
| **B** | 4–6 | The engine: households exist, people act, practices spread. Depends on A and on `AgDR-020` being ratified. |
| **C** | 7–8 | The claims and the determinism fold. Depends on B. This is the ticket that can legitimately fail and produce a finding rather than a fix. |
